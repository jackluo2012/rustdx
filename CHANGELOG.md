...
> 功能边界源自 ShortMind OS Phase 2 全市场回补实战（5215 只 × 120 自然日日线
> / 2140 万行 5 分钟线）的消费侧 workaround 回收：把每个接入方都在重复造的
> 防御逻辑下沉到库内。

**`Client::bars_range` — 任意周期 K 线区间回补（自动翻页）**
- 支持全部 category（0=5m、1=15m、…、9=日线），分钟线等短周期数据的
  区间回补不再需要每个消费方手写翻页循环；
- 内置全市场实测的四重防护：页序无关判停（页内最旧判覆盖）、翻页偏移
  重叠去重、**乱码年份终止**（分钟线历史深度耗尽时服务器返回 2004/2035
  年日期，实测踩坑）、`[begin, end]` 窗口过滤兜底；
- 时间**严格升序**返回（升序契约落实到解析层：`Kline`/`IndexKline` 的
  `parse` 现统一排序；`IndexKline` 同时补上少返回时截断默认填充项）。

**`TcpConfig::recheck_empty` — 静默限流防护（K 线空结果二次确认）**
- TDX 服务端高频请求下会**静默返回空响应**（不报错），与停牌股「窗口内
  本就无 K 线」的真空无法从返回值区分（全市场扫描实测数据污染源头）；
- 开启后 `k` / `bars_range` 空结果时等 300ms 重连重拉一次，仍空才返回空；
  以停牌股 ~300ms/只的代价换取全市场扫描不被限流污染。默认 `false`。

**`TcpConfig::retry_delay_ms` — 重试线性退避**
- `Tcp::retry` / `auto_reconnect` 失败重连前等待 `retry_delay_ms × n`
  （代理/高并发环境立即重试常连续失败，实测 TUN fake-IP 环境首连易
  `EAGAIN`）。建议 200~400；默认 `0` = 立即重试（旧行为）。

**`limit` 模块 — A 股涨跌停域规则（纯函数）**
- `board_of(code)`：按代码前缀判定板块（主板/创业板/科创板/北交所）；
- `limit_up_price` / `limit_down_price`：`昨收 × (1±幅度)` **四舍五入到分**
  （与交易所一致；主板 10%/ST 5%、创业板科创 20%、北交 30%）；
- `limit_status`：封板/炸板/未触板判型（按分比较，免疫浮点误差）；
- `limit_up_streak`：连板高度（最近连续封板天数）；`is_st_name`：ST 识别；
- 实测样本回归：000428 昨收 4.26 → 涨停 4.69 与行情软件精确一致。

**其他**
- `ex_right_reference` 公开：除权参考价 `(昨收×10 − 派现 + 配股价×配股比)
  ÷ (10 + 送转 + 配股比)` 四舍五入到分（与交易所口径一致），
  `k_adjusted` 复权因子内部同步复用（精度与真实除权日昨收对齐）；
- `DateTime` 派生 `Copy/Ord/Hash`：字段顺序即时间序，升序契约的基石。

**⚠️ 破坏性变更（低影响）**
- `TcpConfig` 新增 2 个字段：字面量构造需补 `..Default::default()`
  （`TcpConfig::with_index` / `Default` 不受影响）。

**测试**
- 新增实盘集成测试 `tests/bars_range_live.rs`（升序/去重/窗口/交易时段/
  recheck_empty 回归）；limit 模块 8 个单测含实测样本；全 workspace 测试
  全绿，clippy `-D warnings` 零警告。

## v1.7.0 (2026-09-07)

### 🔧 当日分时 2026 新协议解析修复（rustdx-complete）

- `MinuteTime` 现已**直接解析** 2026 新协议响应（此前只能防御性返回空并回退）：
  头 11 字节（数量+跳过+market+code）+ 可变个数头部 varint（盘口快照：最新价、
  昨收/开/高/低差值、总量等，实测 29~32 个随行情浮动）+ `N×3` 数据 varint，
  数据块恰好耗尽到响应末尾；
- varint 编码非自同步（反向切分不唯一，已实测排除），采用正向穷举头部结束位置，
  以「数据块恰好读满 N×3 个 varint 且结束于响应末尾 + 价格自洽」唯一定位；
- 用今日历史分时（旧协议稳定）交叉验证：沪深主板 4 只股票（000001/600000/
  000858/600519）逐点价格 100% 匹配、成交量一致；
- 新旧格式以头部 `code` 锚点判别（`v[5..11] == 请求代码`），旧协议解析路径保留，
  解析异常仍返回空数据（防御行为不变）；
- 2026-09-07 盘中端到端实测：`Client::minute` 直连解析 206 点，末点与实时行情
  偏差 0.000，首点与开盘价一致；自动回退降级为兜底路径（正常不再触发）；
- 测试：新增实盘集成测试 `tests/minute_time_live.rs`（历史分时对照 + 多服务器
  一致性，`RUSTDX_SKIP_INTEGRATION_TESTS` 可跳过）；单元测试含 2026-09-07
  完整实测响应样本（沪深各一）；lib 127 + 集成全部通过。
...
- `struct_size` 快照随 `Tcp` 结构更新（120 → 128 → 144，TcpConfig 增配字段）；lib 122 + 全部集成测试通过

## v1.5.0 (2026-09-06)

### 🚀 批量 K 线 + 前/后复权（rustdx-complete 新 API）

**`Client::k_batch` — 全市场扫描提速**
- 批量拉取多只股票日 K：内部连接池并行（固定 worker = 连接数，每个连接
  连续处理队列），单只失败不影响其他（错误在单项 `result` 中）；
- 按输入顺序返回；`max_parallel=0` 按 CPU 核数自动，建议 4~16；
- 实测 4 只 × 407 根与逐只 `k()` 结果逐根一致（日期+收盘价）。

**`Client::k_adjusted` / `Adj` — 前/后复权日K**
- 协议实测：K 线请求中 22-24 字节并非复权开关，而是「取样步长」
  （`tests/adj_field_probe.rs`，0x0100 时每 256 根取 1 根），服务器端
  无复权能力 → 复权在本地完成；
- 算法与通达信/akshare 一致：除权参考价 `(preclose*10 - fh + pg*pgj) /
  (10 + pg + sg)`，逐日累积缩放因子；前复权最新价 = 实际价，后复权首日价 =
  实际价；只调整 OHLC，`vol`/`amount` 不变；支持停牌顺延（除权日无 K 线
  时事件顺延到下一交易日）；
- 真实数据验证（sh600519 贵州茅台）：28 个除权日全部消除跳空
  （如 2004-07-01 原始 -26.09% → 复权后回到真实日内波动 -3.05%）。

**测试**
- 新增 4 个复权算法单测（无除权/送转/分红配股/停牌顺延）与 4 个集成测试
  （k_batch 一致性、顺序、复权验证、区间过滤）；lib 122 passed。

## v1.4.0 (2026-09-06)

### 🚀 自动下载健壮性 + 解析校验报告（rustdx-cli）

**下载体验（`day` 自动下载模式）**
- **断点续传**：下载中断后重跑，从已下载位置继续（服务器支持 `Accept-Ranges`，
  实测截断到 256 MiB 后重跑仅续传剩余 ~267 MiB）
- **失败自动重试**：下载失败自动重试 3 次（1s/2s 退避），仍失败才进入交互询问
  备选 URL 或本地 zip 路径
- **缓存跳过**：按 ETag/Last-Modified/Content-Length 比对服务器文件，未变化则
  跳过下载与解压（二次运行实测 **1.09 秒**，不再重复下载 523 MiB / 解压 12,394 文件）；
  缓存按 URL 独立目录，HEAD 探测失败时信任已有下载

**解析校验报告**
- 每次 `day` 解析后自动输出：文件数 / 行数 / 输出大小（MiB）/ 耗时，便于确认
  数据完整性（实测全量格式：`解析完成 → 文件 12394 个 / 行 29632999 行 / ...`）

**测试**
- 新增 `zip_cached` 缓存判定单测（etag 变化 / URL 变化 / 文件缺失）；全部通过
- 目录模式回归通过，校验报告同时覆盖目录模式与自动下载模式
